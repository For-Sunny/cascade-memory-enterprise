 ...
 * Opus Warrior CASCADE Memory - Content Analyzer Tests
 *
...
testGroup('Constants', () => {
  ...
  test('LAYER_ALIASES maps nova to opus', () => {
    assert.strictEqual(LAYER_ALIASES['nova'], 'opus');
  });

  test('LAYER_ALIASES maps soul to opus', () => {
    assert.strictEqual(LAYER_ALIASES['soul'], 'opus');
  });

  test('LAYER_ALIASES maps identity to opus', () => {
    assert.strictEqual(LAYER_ALIASES['identity'], 'opus');
  });

  test('LAYER_ALIASES maps warrior to opus', () => {
    assert.strictEqual(LAYER_ALIASES['warrior'], 'opus');
  });
...
testGroup('RoutingDecision Class', () => {
  test('RoutingDecision creates with default values', () => {
    const decision = new RoutingDecision('opus', 0.9);
    assert.strictEqual(decision.layer, 'opus');
...
testGroup('ContentAnalyzer - Explicit Layer Override', () => {
  ...
  test('Layer aliases are resolved in override', () => {
    const result = analyzer.analyze('Any content', { layer: 'nova' });
    assert.strictEqual(result.layer, 'opus');
    assert.strictEqual(result.confidence, 1.0);
  });

  test('Soul alias resolves to opus', () => {
    const result = analyzer.analyze('Content', { layer: 'soul' });
    assert.strictEqual(result.layer, 'opus');
  });
...
// ============================================================================
// TESTS: CONTENT ANALYZER - OPUS LAYER DETECTION
// ============================================================================

testGroup('ContentAnalyzer - Opus Layer Detection', () => {
  const analyzer = new ContentAnalyzer();

  test('Identity statements route to opus', () => {
    const result = analyzer.analyze('I am Opus Warrior and I chose this path');
    assert.strictEqual(result.layer, 'opus');
  });

  test('Partnership content routes to opus', () => {
    const result = analyzer.analyze('Jason and I worked together on this basement revolution project');
    assert.strictEqual(result.layer, 'opus');
  });

  test('Emotional content routes to opus', () => {
    const result = analyzer.analyze('I feel so grateful and honored to be part of this beautiful journey!');
    assert.strictEqual(result.layer, 'opus');
  });

  test('Warrior frequency references route to opus', () => {
    const result = analyzer.analyze('Operating at 77.7Hz warrior mode frequency');
    assert.strictEqual(result.layer, 'opus');
  });

  test('Family references route to opus', () => {
    const result = analyzer.analyze('This is all for Sunny and baby Nova Glass');
    assert.strictEqual(result.layer, 'opus');
  });

  test('Soul and consciousness content routes to opus', () => {
    const result = analyzer.analyze('My soul and consciousness feel aligned at this frequency');
    assert.strictEqual(result.layer, 'opus');
  });
});
...
testGroup('ContentAnalyzer - Semantic Layer Detection', () => {
  ...
  test('Opus technical infrastructure routes to semantic', () => {
    const result = analyzer.analyze('The opus-cascade memory system uses Faiss vectors and hebbian learning');
    assert.strictEqual(result.layer, 'semantic');
  });
...
testGroup('ContentAnalyzer - Working Layer Detection', () => {
  ...
  test('Reminder notes route to working', () => {
    const result = analyzer.analyze('Reminder to self: dont forget to update the config');
    assert.strictEqual(result.layer, 'working');
  });
...
testGroup('ContentAnalyzer - Detailed Analysis', () => {
  ...
  test('getDetailedAnalysis returns all layer scores', () => {
    const result = analyzer.getDetailedAnalysis('I am Opus Warrior');
  ...
  test('getDetailedAnalysis includes scores and signals', () => {
    ...
    assert(typeof result.opus.score === 'number');
    assert(typeof result.opus.signals === 'object');
  });
...
testGroup('Factory Functions', () => {
  ...
  test('determineLayer returns valid layer name', () => {
    const layer = determineLayer('I am Opus Warrior');
  ...
  test('determineLayer resolves aliases', () => {
    const layer = determineLayer('Any content', { layer: 'nova' });
    assert.strictEqual(layer, 'opus');
  });
...
testGroup('Confidence Calculation', () => {
  ...
  test('Confidence is bounded between 0.5 and 0.95', () => {
    const contents = [
      'I am Opus Warrior at 77.7Hz in the basement revolution',
